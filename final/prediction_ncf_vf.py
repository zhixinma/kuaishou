...
gpu_num = 0
ctx = mx.gpu(gpu_num)
myncf = ncf_vf(nb_users, age_size, score_size)
...
if IS_TRAIN:
	for epoch in range(total_epoch):
		it = 0
		for user_batch, photo_batch, text, user_doc_batch, photo_doc_batch, visual, face_batch, label in batch_generator(train, train_visual, user_doc, photo_doc, batch_size, ind_tr, train_item_num, train_photo_to_index, IS_TRAIN):
			with autograd.record():
				pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(face_batch,ctx=ctx), nd.array(visual, ctx=ctx))
		...
		labels = []
		preds = []
		for user_batch, photo_batch, text, user_doc_batch, photo_doc_batch, visual, face_batch, label in batch_generator(train, train_visual, user_doc, photo_doc, batch_size, ind_te, test_item_num, train_photo_to_index, IS_TRAIN):
			pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(face_batch,ctx=ctx), nd.array(visual, ctx=ctx))
	...
	preds = []
	for user_batch, photo_batch, text, user_doc_batch, photo_doc_batch, visual, face_batch, label in batch_generator(test, test_visual, user_doc, photo_doc, batch_size, ind_res, len_test, test_photo_to_index, IS_TRAIN):
		pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(face_batch,ctx=ctx), nd.array(visual, ctx=ctx))
		preds += pred.asnumpy().reshape((1,-1)).tolist()[0]
	...
	for user_batch, photo_batch, text, user_doc_batch, photo_doc_batch, visual, face_batch, label in batch_generator(train, train_visual, user_doc, photo_doc, batch_size, index, test_item_num+train_item_num, train_photo_to_index, IS_TRAIN):
		pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(face_batch,ctx=ctx), nd.array(visual, ctx=ctx))
		preds += pred.asnumpy().reshape((1,-1)).tolist()[0]
