...
############################## New Partition ################################
# photo_set = np.array(data["photo_id"].unique())
# shuffled_photo_index = range(nb_photos)
# random.shuffle(shuffled_photo_index)
# photo_set_boundary = int(nb_photos*0.1)
# validation_photo = photo_set[shuffled_photo_index[0:photo_set_boundary]]
# validation_index = train["photo_id"].isin(validation_photo)
# ind_te = np.argwhere(validation_index).reshape((-1,))
# ind_tr = np.argwhere(np.logical_not(validation_index)).reshape((-1,))
...
gpu_num = 1
ctx = mx.gpu(gpu_num)
myncf = ncf_ueit(nb_users, nb_photos, nb_words)
myncf.initialize(ctx=ctx)
myncf.collect_params()["ncf_ueit0_embedding0_weight"].set_data(nd.array(userep, ctx=ctx))
