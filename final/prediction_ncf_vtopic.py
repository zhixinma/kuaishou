...
gpu_num = 2
ctx = mx.gpu(gpu_num)
myncf = ncf_vtopic(nb_users, topics_num, MAX_SENTENCE_LENGTH)
...
if IS_TRAIN:
	...
	for epoch in range(total_epoch):
		...
		for user_batch, text, visual, label in batch_generator(train, train_visual, batch_size, ind_tr, train_item_num, train_photo_to_index, IS_TRAIN):
			with autograd.record():
				pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(text, ctx=ctx), nd.array(topics, ctx=ctx), nd.array(visual, ctx=ctx))
		...
		myncf.save_params("model/lda/%s_E%d.params"%(model_name, epoch))
else:
	epoch = 2
	...
	for user_batch, text, visual, label in batch_generator(test, test_visual, batch_size, ind_res, len_test, test_photo_to_index, IS_TRAIN):
		pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(text, ctx=ctx), nd.array(topics, ctx=ctx), nd.array(visual, ctx=ctx))
		preds += pred.asnumpy().reshape((1,-1)).tolist()[0]
	with open('res/%s_E%d.txt'%(model_name, epoch),'wb') as submission:
...
# 		pred = myncf(nd.array(user_batch,ctx=ctx), nd.array(text, ctx=ctx), nd.array(topics, ctx=ctx), nd.array(visual, ctx=ctx))
# 		preds += pred.asnumpy().reshape((1,-1)).tolist()[0]
# 	np.save('res/%s_E%d.npy'%(model_name, epoch), np.array(preds))
